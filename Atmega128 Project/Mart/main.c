...
void calculate_mode(unsigned char key_in)
{
	...
	unsigned long money, unit_money, num,change_money;
	led_light(RED);
	...
	while( 1 ) {
		...
		if( key_flag ) {
			switch( key ) {
				...
				case KEY_9 :
				if( mode != NONE ) {
					if( (mode == INPUT_MODE && pos < 6)
					|| (mode == CHANGE_MODE && pos < 10) ) {
						if( pos == 0 && key == KEY_0 );
						else {
							lcd_gotoxy(pos, 1);
							lcd_data_write(key);
				...
				if( mode == INPUT_MODE ) {
					...
					if( pos ) {
						...
						// 동작
						////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						//money : 지금까지 입력받은 금액이 들어있음.
						//changemode에 입장해서 enter를 눌렀을때의 동작임.
						//pos는 belt_on과 함께 리셋됨.
						//
						unit_money = 0;
						change_money = 0;
						for( i = 0; i < pos; i++ ) {
						...
						//change_money = unit_money - money;//unit_money는 지금 당장 입력받아진 값이니까.
						if(unit_money < money)
						{
							lcd_gotoxy(0,1);
							lcd_string("                    ");
						...
						{
							change_money = unit_money - money;
							lcd_gotoxy(0,1);
							printf("=%10ld won   ", change_money);
							mode = NONE;
							oldsec = sec;
							belt_on =1;//그 카운터 튀어나나오는것.
							servo_spin(90);
							total_money += money;
							int count2 =0;
							while(1)
...
int main(void)
{
	...
	while( 1 ) {
		...
		if( photo_check() ) {
			/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			// [문제 3] “요구사항 (2)”의 상태에서 포토인터럽트에 물체가 감지되면, “Calculate Mode”로 전환되고,
