/*
 * ADC.c
 * 가변저항 값을 조절해서 전압 값(0-5V)을 분해능 10bit 로 0-1023 사이의 값을 출력
 * Created: 2025-05-19 오후 1:22:00
 * Author : me
 */ 
...
int main(void)
{
	DDRF &= ~(1 << PF0); // ADC값 포트 PF0에 받음
	PORTF |= (1 << PF0);
	
	LCD_init();					// 텍스트 LCD 초기화
	LCD_write_string("ADC value");	// 문자열 출력
	...
	while(1){
		...
		adc=read_ADC();
		char adcstr[5] = {};  // 두 자릿수 문자열을 위한 버퍼

		adcstr[1] = '0' + (adc % 10);  // 일의 자리
		adcstr[2] = '0' + (adc / 10) % 10;  // 십의 자리
		adcstr[3] = '0' + (adc / 100) % 10;  // 백의 자리
		adcstr[4] = '0' + (adc / 1000) % 10;  // 천의 자리
		
		// 화면에 보이는 영역은 기본값으로 0~1행, 0~15열로 설정되어 있다.
		LCD_goto_XY(0, 0);			// 0행 0열로 이동
		LCD_write_data(adcstr[4]);		// 문자 단위 출력
		LCD_goto_XY(0, 1);
		LCD_write_data(adcstr[3]);
		LCD_goto_XY(0, 2);
		LCD_write_data(adcstr[2]);
		LCD_goto_XY(0, 3);
		LCD_write_data(adcstr[1]);
		}
	return 0;
}
